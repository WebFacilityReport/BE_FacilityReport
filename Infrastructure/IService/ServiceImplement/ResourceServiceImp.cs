...
public class ResourceServiceImp : IReService
{
    ...
    public async Task<List<ResponseResource>> SearchGetAllResource(string query)
    {
        if (string.IsNullOrEmpty(query))
        {
            var tasks = await _unitofWork.Resource.GetALLResource();
            return _mapper.Map<List<ResponseResource>>(tasks);
        }
        var resource = await _unitofWork.Resource.SearchGetALLResource(query);
        return _mapper.Map<List<ResponseResource>>(resource);
    }
